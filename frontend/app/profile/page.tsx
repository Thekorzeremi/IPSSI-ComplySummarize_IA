...
export default function Profile() {
  const [user, setUser] = useState(mockProfile[0]);
  ...
  function handleDelete() {
    setShowDelete(false);
    setMessage("Compte supprimé (mock)");
  }

  function handleChangeSub(newSub: string) {
    setUser((u) => ({ ...u, subscription: newSub }));
    setForm((f) => ({ ...f, subscription: newSub }));
  }
  ...
  return (
    <div className="max-w-4xl mx-auto mt-12 bg-[#181c2375] rounded-xl shadow-xl border border-white/10 p-0 text-white overflow-hidden">
      <div className="flex border-b border-white/10 bg-[#13161b63]">
        <button
          className={`flex-1 py-4 text-lg hover:cursor-pointer font-bold transition border-b-2 ${tab === "infos" ? "border-white text-white bg-[#4444449d]" : "border-transparent text-gray-300 hover:bg-[#474747b7]"}`}
          onClick={() => setTab("infos")}
        >
          Infos personnelles & options
        </button>
        <button
          className={`flex-1 py-4 text-lg hover:cursor-pointer font-bold transition border-b-2 ${tab === "abos" ? "border-white text-white bg-[#4444449d]" : "border-transparent text-gray-300 hover:bg-[#474747b7]"}`}
          onClick={() => setTab("abos")}
        >
          Abonnements
        </button>
              ...
              <div className="flex gap-4 mt-4">
                {!edit ? (
                  <button
                    ...
                    className="px-6 py-2 rounded-lg border border-white hover:cursor-pointer text-white font-semibold shadow hover:bg-white/25 hover:text-white transition focus:outline-none"
                  >
                    Modifier
                  </button>
                ) : (
                  <>
                    <button
                      type="submit"
                      className="px-6 py-2 rounded-lg border border-white text-white font-semibold shadow hover:bg-white/25 hover:text-white hover:cursor-pointer transition focus:outline-none"
                    >
                      Enregistrer
                    </button>
                    <button
                      type="button"
                      onClick={() => { setEdit(false); setForm({ ...user, password: "" }); setError(""); }}
                      className="px-6 py-2 rounded-lg border border-white text-white font-semibold shadow hover:bg-white/25 hover:text-white hover:cursor-pointer transition focus:outline-none"
                    >
                      Annuler
                    </button>
                ...
                <button
                  ...
                  className="ml-auto px-6 py-2 rounded-lg border border-red-600 text-red-600 font-semibold shadow hover:bg-red-800 hover:text-white hover:cursor-pointer transition"
                >
                ...
                <div className="bg-[#1a1d23] p-8 rounded-xl shadow-xl border border-white/10 max-w-sm w-full">
                  ...
                  <div className="flex gap-4">
                    <button
                      ...
                      className="px-5 py-2 rounded-lg border border-red-600 text-red-600 font-semibold shadow hover:bg-red-800 hover:text-white hover:cursor-pointer transition focus:outline-none"
                    >
                      Oui, supprimer
                    </button>
                    <button
                      onClick={() => setShowDelete(false)}
                      className="px-5 py-2 rounded-lg border border-white text-white font-semibold shadow hover:bg-white/25 hover:text-white hover:cursor-pointer transition focus:outline-none"
                    >
                      Annuler
                    </button>
                  </div>
                ...
                >
                  ...
                  {user.subscription !== sub.id && (
                    <button
                      ...
                      className="mt-auto px-4 py-2 rounded-lg bg-white/20 hover:cursor-pointer text-white font-semibold shadow hover:bg-white/40 transition focus:outline-none focus:ring focus:ring-white"
                    >
                      Choisir cette offre
                    </button>
                  )}
